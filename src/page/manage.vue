<template>
	<div class="manage_page fillcontain">
		<head-top :pageTitle="pageTitle"></head-top>
		<el-row style="height: 100%;" type="flex" class="noWarp">
			<el-col :span="4" style="min-height: 100%;min-width:200px; background-color: #324057;">
				<el-menu style="min-height: 100%;" theme="dark" router>
					<el-menu-item index="/">
						<i class="el-icon-menu"></i>首页</el-menu-item>
					<el-submenu index="2">
						<template slot="title">
							<i class="el-icon-document"></i>管理</template>
						<el-menu-item index="media">素材管理</el-menu-item>
						<el-menu-item index="fans">粉丝管理</el-menu-item>
						<el-menu-item index="">消息管理</el-menu-item>
						<el-menu-item index="">管理员列表</el-menu-item>
					</el-submenu>
					<el-submenu index="3">
						<template slot="title">
							<i class="el-icon-plus"></i>添加文章</template>
						<el-menu-item index="">添加商铺</el-menu-item>
						<el-menu-item index="addGoods">添加商品</el-menu-item>
					</el-submenu>
					<el-submenu index="4">
						<template slot="title">
							<i class="el-icon-star-on"></i>图表</template>
						<el-menu-item index="">用户分布</el-menu-item>
						<!-- <el-menu-item index="newMember">用户数据</el-menu-item> -->
					</el-submenu>
					<el-submenu index="5">
						<template slot="title">
							<i class="el-icon-edit"></i>留言</template>
						<!-- <el-menu-item index="uploadImg">上传图片</el-menu-item> -->
						<el-menu-item index="">留言回复</el-menu-item>
					</el-submenu>
					<el-submenu index="6">
						<template slot="title">
							<i class="el-icon-setting"></i>设置</template>
						<el-menu-item index="">管理员设置</el-menu-item>
						<el-menu-item index="">通知设置</el-menu-item>
					</el-submenu>
					<el-submenu index="7">
						<template slot="title">
							<i class="el-icon-warning"></i>说明</template>
						<el-menu-item index="">说明</el-menu-item>
					</el-submenu>
				</el-menu>
			</el-col>
			<el-col :span="20" class="view-page" >
				<!-- <keep-alive> -->
					<router-view></router-view>
				<!-- </keep-alive> -->
			</el-col>
		</el-row>
	</div>
</template>

<script>
import headTop from "../components/headTop";
export default {
  data() {
    return {
      pageTitle: "A2微信公众号后台管理系统"
    };
  },
  components: {
    headTop
  }
};
</script>


<style lang="less" scoped>
@import "../style/mixin";
.manage_page {
}
.noWarp{
	word-wrap: normal;
}
.view-page {
  height: 100%;
	overflow: auto;
	padding: 2%;
	min-height: 100%;
	min-width:760px; 
}
</style>
